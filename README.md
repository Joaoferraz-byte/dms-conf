...
- **0011 battery-fallback-icon** — changes `BatteryService.getBatteryIcon()` to return `"battery_std"` instead of `"power"` when no battery is available, so batteryless hosts do not show a duplicate power glyph next to the ControlCenterButton power action.
- **0012 focused-app-name-only** — makes the DankBar `FocusedApp` widget show only the application name (e.g. "Firefox"), never the window title, in both compact and non-compact modes. In compact mode `appText` is empty (the name pill is hidden), so the patch resolves the name via `Paths.getAppName(activeWindow.appId, activeDesktopEntry)` directly in `titleText`; in non-compact mode the name is already shown by `appText`, so `titleText` returns an empty string to keep the bar clean and drop the window-title + separator.
- **0013 overview-fill-calendar-space** — repurposes the central area of the DankDash overview tab when no calendar backend is configured. Patch 0007 deactivates the calendar `Loader` when `SettingsData.calendarBackend === "none"`, which left the middle of the overview empty. This patch makes the `MediaOverviewCard` expand leftward into that space (`x = parent.width * 0.2 - Theme.spacingM`, `width = parent.width * 0.8`) when the calendar is inactive, and keeps the original narrow right-column geometry (`x = parent.width * 0.8`, `width = parent.width * 0.2`) when a calendar backend is present, so the overview is never left with an empty middle.
